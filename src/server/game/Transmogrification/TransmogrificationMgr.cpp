...
{
    if (!creature || !creature->IsTransmogrifier())
        return;

    // ASMSG_TRANSMOGRIFICATION_MENU_OPEN\tSlotID:TransEntry;SlotID:TransEntry;...
    std::string message = "ASMSG_TRANSMOGRIFICATION_MENU_OPEN\t" + GenerateTransmogrificationInfoFor(player);
...
void TransmogrificationMgr::HandleTransmogrificationPrepareRequestFrom(Player* player, uint8 pos, uint32 transEntry)
{
    ...
    player->SendAddonMessage("ASMSG_TRANSMOGRIFICATION_PREPARE_RESPONSE\t{}:{}:{}:{}", pos + 1, transEntry, result, cost);
}
...
void TransmogrificationMgr::HandleTransmogrificationApplyRequestFrom(Player* player, std::map<uint8, uint32> data)
{
    ...
    char msg[256];
    snprintf(msg, 256, "ASMSG_TRANSMOGRIFICATION_APPLY_RESPONSE\t%u", result);

    player->SendAddonMessage(msg);
}
